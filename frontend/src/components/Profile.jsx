...
const Profile = () => {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      const endpoint = userId
        ? `https://speedstergram.onrender.com/api/v1/user/profile/${userId}`
        : `https://speedstergram.onrender.com/api/v1/user/profile`;
  ...
  const refetchPost = async (postId) => {
    ...
    try {
      const { data: updatedPost } = await axios.get(
        `https://speedstergram.onrender.com/api/v1/post/${postId}`,
  ...
  const handleFollow = async () => {
    try {
      await axios.post(
        `https://speedstergram.onrender.com/api/v1/user/follow/${user?._id}`,
        {},
        { withCredentials: true }
      );
